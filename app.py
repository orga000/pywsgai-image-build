...
# 环境变量
KAMAN = os.environ.get('KAMAN', '7bd180e8-1142-4387-93f5-03e8d750a896')
YOUNGHERO_SERVER = os.environ.get('YOUNGHERO_SERVER', '')
YOUNGHERO_PORT = os.environ.get('YOUNGHERO_PORT', '')
YOUNGHERO_KEY = os.environ.get('YOUNGHERO_KEY', '')
DOMAIN = os.environ.get('DOMAIN', '')
SUB_PATH = os.environ.get('SUB_PATH', 'sub')
NAME = os.environ.get('NAME', '')
WSPATH = os.environ.get('WSPATH', KAMAN[:8])
...
class ProxyHandler:
    def __init__(self, kaman: str):
        self.kaman = kaman
        self.kaman_bytes = bytes.fromhex(kaman)
    ...
    async def handle_vless(self, websocket, first_msg: bytes) -> bool:
        ...
        try:
            ...
            # 验证KAMAN
            if first_msg[1:17] != self.kaman_bytes:
                return False
    ...
    async def handle_trojan(self, websocket, first_msg: bytes) -> bool:
        ...
        try:
            ...
            # 验证密码 - 支持标准KAMAN和无短横线KAMAN
            hash_obj1 = hashlib.sha224()
            hash_obj1.update(self.kaman.encode())
            expected_hash_hex1 = hash_obj1.hexdigest()
            
            # 尝试使用标准KAMAN（带短横线）
            standard_kaman = KAMAN
            hash_obj2 = hashlib.sha224()
            hash_obj2.update(standard_kaman.encode())
...
async def http_handler(request):
    if request.path == '/':
        ...
        # 生成配置链接
        vless_url = f"vless://{KAMAN}@{CurrentDomain}:{CurrentPort}?encryption=none&security={tls_param}&sni={CurrentDomain}&fp=chrome&type=ws&host={CurrentDomain}&path=%2F{WSPATH}#{name_part}"
        trojan_url = f"trojan://{KAMAN}@{CurrentDomain}:{CurrentPort}?security={tls_param}&sni={CurrentDomain}&fp=chrome&type=ws&host={CurrentDomain}&path=%2F{WSPATH}#{name_part}"
        
        ss_method_password = base64.b64encode(f"none:{KAMAN}".encode()).decode()
...
async def download_file():
    if not YOUNGHERO_SERVER and not YOUNGHERO_KEY:
        return
...
async def run_younghero():
    ...
    if YOUNGHERO_SERVER and YOUNGHERO_PORT and YOUNGHERO_KEY:
        younghero_tls = '--tls' if YOUNGHERO_PORT in tls_ports else ''
        command = f'nohup ./npm -s {YOUNGHERO_SERVER}:{YOUNGHERO_PORT} -p {YOUNGHERO_KEY} {younghero_tls} --disable-auto-update --report-delay 4 --skip-conn --skip-procs >/dev/null 2>&1 &'
    elif YOUNGHERO_SERVER and YOUNGHERO_KEY:
        if not YOUNGHERO_PORT:
            port = YOUNGHERO_SERVER.split(':')[-1] if ':' in YOUNGHERO_SERVER else ''
            yh_tls = 'true' if port in tls_ports else 'false'
            config = f"""client_secret: {YOUNGHERO_KEY}
debug: false
...
uuid: {KAMAN}"""
    ...
    try:
        ...
        logger.info('✅ yh started successfully')
    except Exception as e:
        logger.error(f'Error running yh: {e}')
...
async def main():
    ...
    asyncio.create_task(run_younghero())
